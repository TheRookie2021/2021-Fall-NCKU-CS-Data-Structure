                          ...
                          bool verbose)
{
    ...
    int returned_keys[3000];
    void *returned_pointers[3000];
    int num_found = find_range(root, key_start, key_end-1, verbose,
                               returned_keys, returned_pointers);

    printf("range [%d,%d) = ",key_start,key_end);
    if (!num_found||(key_start == key_end))
        printf("[]\n");
    else
    {
        printf("[");
...
int main()
{
    ...
    for (int i = 0; i < op_n; i++)
    {
        ...
        {
            scanf("%d", &variable_a);
            root = insert(root, variable_a, count++);
        }
        else if (strcmp(op, "remove") == 0)
        {
            scanf("%d", &variable_a);
            root = delete (root, variable_a);
        }
        else if (strcmp(op, "range-search") == 0)
        {
            scanf("%d%d", &variable_a, &variable_b);
            find_and_print_range(root, variable_a, variable_b, false);
        }
